...
a<- ggplot(data = Data, aes(Outs, RunScr))+
  ...
  annotation_custom(g_pic1, xmin =4.5, ymin = 80, xmax = 5, ymax = 100)
...
b<- ggplot(data = Data2, aes(Outs, RunScr))+
  ...
  theme(panel.background=element_rect("#141622"),
        ...
        legend.position = "FALSE")+
  scale_x_continuous(breaks = c(0,1,2,3,4,5))+
  annotation_custom(g_pic2, xmin =4.5, ymin = 80, xmax = 5, ymax = 100)

#add competition logos
img1<- png::readPNG("BBL.png")
g_pic1<- rasterGrob(img1, interpolate=TRUE)

img<- png::readPNG("IPL2.png")
g_pic2<- rasterGrob(img, interpolate=TRUE)
