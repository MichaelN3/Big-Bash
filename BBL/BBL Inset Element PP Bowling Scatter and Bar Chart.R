...
a<- ggplot(Data, aes(ER, BallsperWicket))+
  ...
  geom_text_repel(data = Data%>%filter(ER >= 9.5 | BallsperWicket>40), aes(label = player), color = "white", family = "Fira Sans", size = 2.8)+
  ...
  annotate("label", x = 9.8, y = 10, label = "High Economy Rate,\nLow Strike Rate",
           family = "Fira Sans", size = 3, fill = "gray78")+
  annotate("label",x = 6.2, y = 45, label = "Low Economy Rate,\nHigh Strike Rate",
           family = "Fira Sans", size = 3, fill = "gray78")+
...
a + inset_element(b, left = 0.63, right = 0.98, 
                  bottom = 0.58, top = 0.98,
                  align_to = "full") 
