...
geom_mark_circle(aes(filter = player %in% c("Moeen Ali"),
                       label = "Moeen Ali",
                       description = "Strike Rate of 187\nwith 54% of runs hit in\nsixes and 22% in fours"),
                   label.fontsize = c(13,10),
                   label.family = "Roboto Condensed",
                   label.buffer = unit(10, "mm"))+
  geom_mark_circle(aes(filter=player=="Andre Russell",
                       label = "Andre Russell",
                       description = "Boundary-hitter,\naveraging a boundary\nevery 4 balls\nbut high dot ball %"),
                   label.fontsize = c(13,10),
                   label.family = "Roboto Condensed")+
  geom_mark_circle(aes(filter=player=="Jordan Silk",
                       label = "Jordan Silk",
                       description = "Strike-rotater, with a\nlow dot ball% but has\naveraged a boundary\nevery 14 balls"),
                   label.fontsize = c(13,10),
                   label.family = "Roboto Condensed")+
